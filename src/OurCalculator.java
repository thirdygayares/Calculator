...
public class OurCalculator implements ActionListener{
	...
	public OurCalculator() {
		...
		history = new JButton("HS"); // create route for this 
		...
		historyLog.setBounds(0,178,335,500);
		
		
		 historyList = new JPanel();
		
		historyList.setBounds(0,100,335,500);
		
		historyList.setLayout(new GridLayout(10,1,1,1));
		...
		historyLog.add(historyList);
		Frame.add(historyLog);
		historyLog.setBackground(Color.decode("#FFD4D4"));
		historyLog.setLayout(null);
		...
		historyText.setFont(f5);
		historyLog.add(historyText);
		historyText.setBounds(20,20,120,50);
		...
		historyLog.add(back);
		back.setBounds(210,20,100,50);
		back.setFocusable(false);
		back.setBackground(Color.decode("#D06666"));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()== equals) {
			...
			historyStorage[counter] = viewNumber1.getText().concat(viewNumber2.getText());
		...
		if(e.getSource()==history) {
			...
			for(int i=0; i<10;	i++) {
				historyComputation[i] = new JLabel(historyStorage[i]);
				historyComputation[i].setFont(new Font("arial", Font.PLAIN, 20));
				historyList.add(historyComputation[i]);	
			}
	...
	public static void main(String[] args) {
		...
		System.out.println(historyStorage[0]);
	}
}